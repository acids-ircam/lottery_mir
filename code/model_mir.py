...
###################
"""
class LotteryRegression(LotteryClassification):
    
    def __init__(self, args):
        super(LotteryRegression, self).__init__(args)
        self.pruning = args.pruning


# Recurrent neural network (many-to-one)
class CREPEModel(LotteryRegression):
    
    capacity = {'tiny': 4, 'small': 8, 'medium': 16, 'large': 24, 'full': 32}
    
    def __init__(self, args):
        super(CREPEModel, self).__init__(args)
        capacity_m = self.capacity[args.capacity]
        layers = [1, 2, 3, 4, 5, 6]
        filters = [n * capacity_m for n in [32, 4, 4, 4, 8, 16]]
