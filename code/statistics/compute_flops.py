...
def compute_flops(module, inp, out):
    ...
    if isinstance(module, nn.Conv1d):
        ...
        return compute_LSTM_flops(module, inp, out)
    elif isinstance(module, nn.GRU):
        return compute_GRU_flops(module, inp, out)
    elif isinstance(module, nn.LSTM):
        return compute_GRU_flops(module, inp, out)
    else:
...
def compute_Linear_flops(module, inp, out):
    ...
    batch_size = inp.size()[0]
    if (len(inp.shape) == 1):
        feat_size = inp.size()[0]
    else:
        feat_size = inp.size()[1]
    return batch_size * feat_size * out.size()[1]
...
def compute_LSTM_flops(module, inp, out):
    ...
    bidir = module.bidirectional and 2 or 1
    return module.num_layers * bidir * 8 * 2 * hidden_size * (hidden_size + 1)

def compute_GRU_flops(module, inp, out):
    assert isinstance(module, nn.GRU)
    #elements = inp.size()[0]
    #for s in inp.shape[1:]:
    #    elements *= s
    hidden_size = module.weight_ih_l0.shape[0]
    bidir = module.bidirectional and 2 or 1
    return module.num_layers * bidir * 6 * 2 * hidden_size * (hidden_size + 1)


def compute_RNN_flops(module, inp, out):
    assert isinstance(module, nn.RNN)
    #elements = inp.size()[0]
    #for s in inp.shape[1:]:
    #    elements *= s
    hidden_size = module.weight_ih_l0.shape[0]
    bidir = module.bidirectional and 2 or 1
    return module.num_layers * bidir * 2 * 2 * hidden_size * (hidden_size + 1)
