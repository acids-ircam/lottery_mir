...
nb_epochs = 200
# Set of datasets
datasets = ['mnist', 'nsynth-10000', 'sol-styles', 'sol-ordinario']
# Models grid arguments
model = ['ddsp', 'sing-ae', 'wavenet']
...
def extract_results(analyzer, load_models=False, properties=None, name=None):
    ...
    if (x == 's'):
        return None
    if (len(final_stats['test_loss']) < 100):
        return None
    final_stats['test_loss'] = final_stats['test_loss'][:100]
    final_stats['prune_ratio'] = [] #final_stats['prune_ratio'][:12]
    final_stats['train_full'] = [] #final_stats['prune_ratio'][:12]
    final_stats['test_full'] = [] #final_stats['prune_ratio'][:12]
    final_stats['valid_full'] = [] #final_stats['prune_ratio'][:12]
    final_stats['train_loss'] = [] #final_stats['prune_ratio'][:12]
    final_stats['valid_loss'] = [] #final_stats['prune_ratio'][:12]
    return final_stats
...
######
"""
base_results = 'output/waveform_final'
full_results = {}
